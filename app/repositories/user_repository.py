...
def get_user_by_supabase_id(db: Session, sub: str) -> models.User | None:
    """
    supabase_id corresponds to the JWT 'sub' claim.
    This identifies the same user across login sessions.
    """
    return db.query(models.User).filter_by(supabase_id=sub).first()

def create_user(
    db: Session,
    email: str,
    supabase_id: str | None = None,
    role: models.UserRole = models.UserRole.BUYER,
) -> models.User:
    """
...
def get_or_create_user_by_supabase_id(
    ...
    """
    user = get_user_by_supabase_id(db, sub)
    if user:
        return user
    """
    ...
    return create_user(db, supabase_id=sub, email=email, role=role_enum)